...
public class DE_Best_know {
    ...
    public DE_Best_know(int NP, int D, int numF, double CR,  double ab[][]) {
        this.NP = NP;
        this.numF = numF;
        this.D = D;
        this.CR = CR;
...
//            Fs.add(bestOne.fx);//put fixed 
    ...
    public static void main(int changesParam, int dimensionParam, int runsParam, int constraintParam, int lowerParam, int upperParam, int functionParam[], int algorithmParam[], int frequencyParam, int freqDistParam, int accuracyParam, int solverRunsParam) {
        String[] Sfuncion = {"Sphere", "Rastrigin", "Ackley", "Rosenbrock", "Power", "Custom"};
        DecimalFormat df = new DecimalFormat("0.000");
        
        //variables
        int MAX_Functions = 6, runs = solverRunsParam;// variables for the experiments 
        int NP = 20, D = dimensionParam, eval, numVec = 3, gen;
        double TIMEs = changesParam, t = 0, CR = 0.3, F, betamin = .2, betamax = .8;
        double old[];
        int MAX_EVAL = accuracyParam;
        List<Double> ListFXs;
            ...
            if (functionParam[numF] == 1) {
                ...
                for (int time = 0; time < (int) TIMEs; time++) {
                    ...
                    for (int r = 0; r < runs; r++) {
                        ...
                        gen = 0;
                        eval = 0;
                        DE_Best_know DE = new DE_Best_know(NP, D, numF, CR, ab);// A random population was created
                        ...
                        while (eval < MAX_EVAL) {
                            ...
                            if (eval > MAX_EVAL-DE.NP || (accuracyParam > 1000000 && (bestOne.fx < 0.00001 || (eval % 5000 == 0 && old[r] - bestOne.fx < 0.0000000000001)))){    
